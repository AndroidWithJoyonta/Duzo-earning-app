...
class Admob {
    ...
    fun loadNative(view: FrameLayout) {
        val v: View = LayoutInflater.from(view.context).inflate(R.layout.native_ad, null)
    ...
    private fun populateNativeAdView(nativeAd: NativeAd, nativeAdView: NativeAdView) {
        nativeAdView.mediaView = nativeAdView.findViewById<MediaView>(R.id.media_view)
        nativeAdView.callToActionView = nativeAdView.findViewById<View>(R.id.cta)
        nativeAdView.headlineView = nativeAdView.findViewById<View>(R.id.primary)
        nativeAdView.advertiserView = nativeAdView.findViewById<View>(R.id.secondary)
        nativeAdView.starRatingView = nativeAdView.findViewById<View>(R.id.rating_bar)
        nativeAdView.bodyView = nativeAdView.findViewById<View>(R.id.body)
        nativeAdView.iconView = nativeAdView.findViewById<View>(R.id.icon)
        (Objects.requireNonNull(nativeAdView.headlineView) as AppCompatTextView).text =
            nativeAd.headline
        (Objects.requireNonNull(nativeAdView.bodyView) as AppCompatTextView).text =
            nativeAd.body
        (Objects.requireNonNull(nativeAdView.callToActionView) as AppCompatButton).text =
            nativeAd.callToAction
        Objects.requireNonNull(nativeAdView.mediaView)?.mediaContent =
        ...
        if (nativeAd.icon == null) {
            Objects.requireNonNull(nativeAdView.iconView)?.visibility =
                View.INVISIBLE
        } else {
            (Objects.requireNonNull(nativeAdView.iconView) as AppCompatImageView).setImageDrawable(
                nativeAd.icon!!.drawable
            )
            nativeAdView.iconView!!.visibility = View.VISIBLE
        }
        if (nativeAd.starRating == null) {
            Objects.requireNonNull(nativeAdView.starRatingView)?.visibility =
                View.INVISIBLE
        } else {
            (Objects.requireNonNull(nativeAdView.starRatingView) as RatingBar).rating =
                nativeAd.starRating!!.toFloat()
            nativeAdView.starRatingView!!.visibility = View.VISIBLE
        }
        if (nativeAd.advertiser == null) {
            Objects.requireNonNull(nativeAdView.advertiserView)?.visibility =
                View.INVISIBLE
        } else {
            (Objects.requireNonNull(nativeAdView.advertiserView) as AppCompatTextView).text =
                nativeAd.advertiser
    ...
    class AOManager(private val application: Application) : ActivityLifecycleCallbacks,
    ...
    companion object {
       /* var AppOpen: String = "ca-app-pub-1966206743776147~5136407102"
        var Banner: String = "ca-app-pub-1966206743776147/8369075100"

        var Interstitial: String = "ca-app-pub-1966206743776147/1803666755"

        var Rewarded: String = "ca-app-pub-1966206743776147/8177503415"

        var Native: String = "ca-app-pub-1966206743776147/3550467469"*/
